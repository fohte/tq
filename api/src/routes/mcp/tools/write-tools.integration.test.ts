...
describe('create_page tool', () => {
  it('creates a page with the given fields', async () => {
  ...
  it('defaults content and sortOrder when omitted', async () => {
...
describe('update_page tool', () => {
  it('partially updates the given fields', async () => {
    const task = await createTask('Task with notes')
    // Created (rather than fetched) through the MCP tool itself so the
    // create and update edits share the same author — `getPageAuthors`
    // picks the most recent edit by `updatedAt`, which Postgres pins to
    // transaction start, so within this suite's single-transaction-per-test
    // setup a create/update pair from different authors would tie.
