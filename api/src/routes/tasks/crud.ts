...
export const tasksCrudApp = new Hono()
  .post('/', zValidator('json', createTaskSchema), async (c) => {
    ...
    const { task, createdRule, labelNames } = await db.transaction(
      async (tx) => {
        // Setting a recurrence rule on task create redirects into the
        // template model: a template owns the rule, and this task becomes
        // its first generated instance (`templateId`/`occurrenceDate` set,
        // `recurrenceRuleId` left null).
        let templateFields: {
          templateId: string
  ...
  .patch(
    ...
    async (c) => {
      ...
      // Once a task is linked to a template (generated by the scheduler, or
      // turned into a template's first instance by a previous PATCH),
      // further recurrence edits must go through the template.
