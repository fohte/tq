...
`errorHandling` in `eslint.config.js` bans `throw`/`try-catch` in production code and requires every returned `Result` to be consumed (`no-restricted-syntax`, `neverthrow/must-use-result` in `@fohte/eslint-config`). Both rules are enforced only in `api/**` (including api's own config files). `cli/`'s shared helpers already return `Result`/`ResultAsync`, but its command handlers are still throw-based, so both rules stay off there until that migration completes. `web/` and root config files still use `throw`/`try-catch` and don't import `neverthrow`. Return a `Result`/`ResultAsync` from [neverthrow](https://github.com/supermacro/neverthrow) instead:
